...
WOUT  = '/mnt/c/Users/TUSHAR/TUSHAR DUDEJA/SELF PLAY/notch2_wins_both_fire'
WANT  = 1
...
found, seed = 0, 7500
print("scanning for blue-WIN engagements where RED FIRES...")
while found < WANT and seed < 8400:
        ...
        for m in r.ammo.values():
            if m.target_lost and id(m) not in seen:
                if m.notch_count > m.missile_performance.notch_count:
                    ...
                    seen[id(m)] = 'k'; n_kin += 1
    oc = info.get('outcome', '?')
    # KEY CHECK: Both win AND red fired
    if oc == 'win' and rsh > 0:
        ...
        found += 1
        name = (f"blue_win_both_fire_seed{seed}_B{bsh}R{rsh}"
    ...
    seed += 1
print(f"\n{found} blue-win duels saved to {WOUT}")
