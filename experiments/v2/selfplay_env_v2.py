...
class SelfPlayBVR(BVRBase):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.red_prev_action = None
        self._caps_prev = None
        self._wasted_shots = 0
        self._phi_prev = None        # PBRS state (BVRBase.get_reward needs these)
        self._blue_shots = 0
    ...
    def _obs_vec(self, agent, target):
        ...
        # incoming-threat features for `agent` (mirror of BVRBase.from_obs2nn so
        # the opponent sees the SAME 19-dim layout the learner does): the threat
        # is the TARGET's active missile homing on `agent`.
        mis = target.get_active_missile()
        if mis is not None:
            ego = agent.simObj
            te, tn, tu = pm.geodetic2enu(
                mis.get_lat_gc_deg(), mis.get_long_gc_deg(), mis.get_altitude(),
                ego.get_lat_gc_deg(), ego.get_long_gc_deg(), ego.get_altitude())
            t_bearing = to_360(np.degrees(np.arctan2(te, tn)))
            t_range   = float(np.sqrt(te*te + tn*tn + tu*tu))
            t_los     = mis.target_los_speed()
        else:
            t_bearing = heading; t_range = 120e3; t_los = 0.0
        t_rel = np.radians(t_bearing - heading)
        ...
        v = np.array([
            ...
            np.sin(t_rel), np.cos(t_rel),
            scale_between(t_range, 0.0, 120e3),
            np.clip(t_los / 400.0, 0.0, 1.0) * 2.0 - 1.0,
        ], dtype=np.float32)
    ...
    def step(self, action):
        # EMA smoothing removed: the proportional-bank autopilot killed the
        # wobble at the source; the CAPS penalty (raw consecutive actions)
        # handles command smoothness — same scheme as BVRBase.step.
        caps_pen = 0.0
        ...
        action_to_apply = self.from_nn2agent(action, self.blue_agent)

        # red's action from its frozen policy (or BT).
        # deterministic=True: the mean policy IS the skilled one (measured
        # 76% det vs 14% stochastic) — sampling would make opponents pushovers.
        red_action = None
        if self._opp_policy != 'BT':
            r_a, _ = self._opp_policy.predict(self.red_state[None], deterministic=True)
            raw_r_a = np.array(r_a[0], dtype=np.float32)
            red_action = self.from_nn2agent(raw_r_a, self.red_agent)

        for _ in range(self.conf.step_length):
            self.blue_agent.apply_action(action_to_apply)
            if self._opp_policy == 'BT':
